...
export function SessionsTable({ branchId, scope, mine }: { branchId?: number; scope: 'active' | 'recent'; mine?: boolean }) {
  ...
  const m = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('Session terminated'), t('The user has been signed out on that device.'));
      setRevoke(null);
    ...
    <>
      <DataTable
        ...
        columns={[
          {
            key: 'fullName',
            header: t('Account signed in'),
            render: (r) => (
              <div>
                <div className="flex items-center gap-1.5 font-medium">
                  {r.fullName}
                  {r.isCurrent && <Badge>{t('this session')}</Badge>}
                </div>
                <div className="font-mono text-[11.5px] text-ink-500">{r.username}</div>
              </div>
            ),
          },
          { key: 'roleName', header: t('Role'), render: (r) => t(r.roleName) },
          { key: 'branchName', header: t('Branch'), render: (r) => (r.branchName ? t(r.branchName) : t('All branches')) },
          { key: 'loginAt', header: t('Logged in'), render: (r) => <span title={dateTime(r.loginAt, lang)}>{time(r.loginAt, lang)} <span className="text-ink-400">· {dateTime(r.loginAt, lang).split(',')[0]}</span></span> },
          { key: 'lastActivityAt', header: t('Last activity'), render: (r) => <span title={dateTime(r.lastActivityAt, lang)}>{relative(r.lastActivityAt)}</span> },
          { key: 'currentModule', header: t('Current module'), render: (r) => (r.status === 'ACTIVE' ? <span className="rounded bg-canvas px-1.5 py-0.5 text-[12px]">{humanize(r.currentModule)}</span> : '—') },
          {
            key: 'device',
          ...
          { key: 'ipAddress', header: t('IP address'), render: (r) => <Mono className="text-ink-600">{r.ipAddress}</Mono> },
          {
            key: 'presence',
            header: t('Status'),
            render: (r) => (
              <div className="flex flex-col items-start gap-1">
                ...
                {r.concurrentSessions > 1 && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-800" title={t('The same account is signed in on more than one device')}>
                    <AlertTriangle className="size-3" /> {r.concurrentSessions} {t('concurrent')}
                  </span>
                )}
              </div>
            ),
          },
          { key: 'ref', header: t('Session'), render: (r) => <Mono className="text-[11.5px] text-ink-500">{r.ref}</Mono> },
  ...
  return (
    <div className="p-5 lg:p-6">
      <PageHeader
        ...
        subtitle={t('Who is signed in right now, from which device, and what they are working on. Always shows the account that actually signed in.')}
      ...
      {concurrent.size > 0 && (
        <Alert tone="warning" icon={<AlertTriangle className="size-4" />} className="mb-4" title={t('Same account signed in on multiple devices')}>
          {t('{users}: this may indicate shared credentials. Every action is attributed to the account that performed it.', { users: [...concurrent].join('، ') })}
        </Alert>
      )}
      <Card padded={false}>
        <Tabs className="px-3" value={scope} onChange={setScope} tabs={[{ value: 'active', label: t('Active Sessions') }, { value: 'recent', label: t('Last 7 days') }]} />
        <SessionsTable branchId={branchId} scope={scope} />
      </Card>
      <p className={clsx('mt-3 flex items-center gap-1.5 text-[12px] text-ink-500')}>
        <Eye className="size-3.5" /> {t('Transparent monitoring: every user can see their own session details in the user menu. Only sign-in metadata and the current module are recorded. No screen or keystroke capture.')}
      </p>
    </div>
  );
}
