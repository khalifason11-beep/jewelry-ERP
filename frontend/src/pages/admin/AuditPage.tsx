  ...
  return (
    <div className="p-5 lg:p-6">
      <PageHeader
        ...
        subtitle="Append-only record of every important action: who, when, where, and on what."
      />
      <Card padded={false}>
        <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-2.5">
          ...
          <Select value={action} onChange={(e) => setAction(e.target.value)} className="h-8 w-56 text-[13px]" aria-label="Action">
            <option value="">All actions</option>
            {AUDIT_ACTIONS.map((a) => <option key={a} value={a}>{a}</option>)}
          </Select>
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search description, ID, user…" className="h-8 w-56 text-[13px]" />
        </div>
        {data.isLoading ? (
          <Loading />
        ) : data.isError ? (
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: 'branchName', header: t('Branch'), render: (r) => r.branchName ?? <span className="text-ink-400">Company</span> },
              { key: 'action', header: 'Action', render: (r) => <StatusBadge status={r.action} className="font-mono !text-[10.5px]" /> },
              { key: 'entityId', header: 'Entity', render: (r) => (r.entityId ? <span className="text-[12px]"><span className="text-ink-500">{r.entityType}</span> <Mono>{r.entityId}</Mono></span> : '—') },
              { key: 'description', header: t('Description'), className: 'min-w-[320px]' },
              { key: 'ipAddress', header: 'IP / session', render: (r) => <div className="font-mono text-[11px] text-ink-500">{r.ipAddress ?? '—'}<br />{r.sessionRef}</div> },
            ]}
          />
        )}
      </Card>
