...
export function ExpensesTable({ branchId, from, to, toolbar, status }: { branchId?: number; from: string; to: string; toolbar?: React.ReactNode; status?: string }) {
  ...
  return (
    <DataTable
      ...
      exportName="expenses"
      toolbar={toolbar}
      emptyTitle={t('No expenses in this period')}
      columns={[
        { key: 'number', header: t('Expense'), render: (r) => <Mono className="text-ink-900">{r.number}</Mono> },
...
export function ExpensesPage() {
  ...
  return (
    <div className="p-5 lg:p-6">
      <PageHeader
        title={t('Expenses')}
        subtitle={t('Branch operating expenses. Approved expenses reduce branch contribution in all reports.')}
        actions={can('expenses.create') && <Button variant="primary" icon={<Plus className="size-4" />} onClick={() => setOpen(true)}>{t('New expense')}</Button>}
      />
      <Card padded={false}>
        <ExpensesTable
          branchId={branchId}
...
function NewExpenseDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      title={t('New expense')}
      footer={<><Button onClick={onClose}>{t('Cancel')}</Button><Button variant="primary" loading={m.isPending} disabled={!Number(f.amount) || f.description.trim().length < 2 || (isGlobal && !f.branchId)} onClick={() => m.mutate()}>{t('Save expense')}</Button></>}
    >
      <div className="grid gap-3 sm:grid-cols-2">
        {isGlobal && (
          <Field label={t('Branch')} className="sm:col-span-2">
      ...
      {!can('expenses.approve') && <Alert tone="info" className="mt-3">{t('Expenses above the approval threshold are sent to the General Manager for approval.')}</Alert>}
    </Dialog>
  );
}
