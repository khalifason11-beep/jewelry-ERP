  ...
  return (
    <div className="p-5 lg:p-6">
        ...
        <Card className="md:col-span-2">
          ...
          {me.session && (
            <KeyValue
              cols={4}
              items={[
                { label: t('Session'), value: <Mono>{me.session.ref}</Mono> },
                { label: t('Signed in'), value: dateTime(me.session.loginAt, lang) },
                { label: t('Device'), value: deviceText(me.session.device) },
                { label: t('IP address'), value: <Mono>{me.session.ipAddress}</Mono> },
              ]}
            />
          )}
        </Card>
      ...
      <Card padded={false} className="mb-5">
        <CardHeader title={t('My transactions')} />
        <SalesTable mine from={from} to={to} toolbar={<DateRange from={from} to={to} onChange={(r) => set(r)} />} />
      </Card>
      <Card padded={false}>
        <CardHeader title={t('My sessions (last 7 days)')} subtitle={t('The same information administrators see about your account')} />
